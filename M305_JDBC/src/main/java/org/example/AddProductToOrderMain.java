...
public class AddProductToOrderMain {
    ...
    public void run() {
        ...
        // the annotations and the FK value from inside the order object to create and execute the query on our
        // behalf without us doing anything.
        // these 2 lines of code do the same thing.   First line hibernate does the work for you
        // the second line of code (commented) actually does the exact same thing accept manually doing the query
        // with the FK id
        ...
        // NOTE - this way of looping over the products is very inefficient on the database because it runs a lot of
        // queries
        // this same thing could be accomplished with a single custom query that returns columns from multiple
        // different tables
    ...
    // we can just set the product and order objects we just queried on the order detail
        ...
        orderDetail.setOrderLineNumber(112);

    // save the order details to the database
    // it is important to know ... if the id is null then you need to insert it
    // if the record was found in the database then the id will be not null so that is how we know it is an update
    // rather than an insert
        if(orderDetail.getId()==null)

    {
        // hibernate will automatically populate the id as part of the insert and it will be the new id generated by
        // the auto increment
